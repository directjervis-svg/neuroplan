import { int, mysqlEnum, mysqlTable, text, timestamp, varchar, boolean, json, date, time } from "drizzle-orm/mysql-core";
...
export const userStreaks = mysqlTable("user_streaks", {
  ...
  totalDaysActive: int("total_days_active").default(0).notNull(),
...
// ========================================
// TABELA: USER REMINDERS
// Sistema de lembretes configuráveis
// ========================================
export const userReminders = mysqlTable("user_reminders", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
  reminderType: mysqlEnum("reminder_type", [
    "daily_start",      // Lembrete matinal para começar o dia
    "task_pending",     // Lembrete de tarefa pendente
    "cycle_end",        // Lembrete de fim de ciclo
    "weekly_summary"    // Resumo semanal
  ]).notNull(),
  time: time("time").notNull(),                    // Horário do lembrete (HH:MM:SS)
  ...
  enabled: boolean("enabled").default(false).notNull(),  // PADRÃO: DESLIGADO
...
export type NewUserReminder = typeof userReminders.$inferInsert;


// ========================================
// TABELA: SESSION NOTES
// Notas de sessão para "Onde Parei"
// ========================================
export const sessionNotes = mysqlTable("session_notes", {
  ...
  taskContext: json("task_context").$type<{
    lastTaskId?: number;
...
// ========================================
// TABELA: QUICK IDEAS (já existe, apenas documentando)
// ========================================
